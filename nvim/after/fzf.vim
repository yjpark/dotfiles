set runtimepath+=~/.external/tools/fzf
...
" filter a list of vim's commandline auto-completions. Try :colo␣<c-l> (be
" sure to include the space) or :b␣<c-l>. There are special cases for handling
" file-searches, so that you can go deeper into the path than just one
" directory at a time, and so that you can add multiple files to the arglist
" at once. More special cases could be added. Some limitations: the
" auto-complete for :help and :tag are limited to 300 entries, so you may need
" to narrow it a bit.
